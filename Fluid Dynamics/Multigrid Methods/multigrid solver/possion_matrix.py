...
class possion_matrix:
    ...
    def __init__(self, field):
        ...
        self.m = field.m
        self.levels = 0
        self.A, self.Rj, self.invD, self.levels = levels(self.n, self.m)
    
    def get_level_matrices(self, level):
        return self.A[level], self.Rj[level], self.invD[level]
...
def levels(n,m):
    ...
    level = 0
    while next_level:
        A_new, Rj_new, invD_new = create_jacobi_update_arrays(n,m)
        shape.append([n,m])
        A.append(A_new)
        Rj.append(Rj_new)
